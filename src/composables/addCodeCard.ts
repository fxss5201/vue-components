...
function lineNumbersCode(preCode: string) {
  ...
  const lineNumbers = lines.filter(x => x.length > 0).map((_, i) => i + 1)
...
export default function addCodeCard(codeHtml: string, preCode: string, lang: string = '', lineNumbers: boolean = true) {
  let langHtml = ''
  if (lang) {
  ...
  <div class="code-card-tool" style="color: ${colorStyle}">
    ...
    <svg class="copy-btn" data-clipboard-text="${preCode}"
