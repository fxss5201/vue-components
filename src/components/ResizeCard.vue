<template>
  <div class="resize-card"
    ...
    <slot />
    <div class="resize-card__resize" v-if="props.isResize" ref="handleResizeEl" :style="{ right: `${handleResizeElRight + 2}px` }">
...
const props = withDefaults(
  ...
  }>(),
  {
    ...
    maxHeight: 800,
    isResize: true,
    // 是否自动监听滚动条的变化，默认自动监听调整右下角拖拽图标位置，设置为false时，需要手动设置isScroll属性
...
const emit = defineEmits<{
  resize: [value: { width: number; height: number }]
}>()
